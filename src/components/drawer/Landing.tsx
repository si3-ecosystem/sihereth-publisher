...
const LandingFields: React.FC<LandingFieldsProps> = ({ toggleDrawer }) => {
  ...
  const handleInputChange = (fieldName: keyof typeof websiteData.landing, value: string) => {
    const updatedLandingData = { ...websiteData.landing, [fieldName]: value };
    // dispatch(handleWebsiteData({ ...websiteData, landing: updatedLandingData }));
  };
  ...
  const handleCategoryArrayInputChange = (
    value: string,
    textIndex: number,
    categoryKey: keyof typeof websiteData.landing.categories
  ) => {
  ...
  const handleCategoryInputChange = (field: keyof typeof websiteData.landing.categories, value: string) => {
  ...
  const addCategoryTag = (categoryKey: keyof typeof websiteData.landing.categories, maxTags: number) => {
  ...
  const deleteCategoryTextIndex = (textIndex: number, categoryKey: keyof typeof websiteData.landing.categories) => {
    if (websiteData?.landing?.categories[categoryKey].length === 1) {
      return;
    }
    const updatedCategories = {
      ...websiteData?.landing?.categories,
      [categoryKey]: websiteData?.landing?.categories[categoryKey].filter((ITEM, INDEX) => INDEX !== textIndex)
    };
    // dispatch(
    //   handleWebsiteData({
    //     ...websiteData,
          ...
          <div className="flex flex-col items-start p-4 mt-4">
            ...
            <div className="w-full">
              <input
                ...
                disabled
                id="hashTagTitle"
                className="block p-2.5 mt-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 cursor-not-allowed hover:ring-gray-300 hover:border-gray-400 focus:ring-gray-300 focus:border-gray-400"
                  ...
                  <div className="mx-auto mt-1 space-y-4 max-w-3xl">
                    ...
                    <div className="w-full">
                      ...
                      {websiteData?.landing?.categories?.superPower?.map((item, textIndex) => {
                        return (
                          <div key={textIndex} className="flex gap-4 items-center">
                            <input
                              value={item}
                              type="text"
                              id="hashTagTitle"
                              ...
                              required
                              onChange={(e) => handleCategoryArrayInputChange(e.target.value, textIndex, "superPower")}
                            />
                            <RiDeleteBinLine
                    ...
                    <div className="w-full">
                      <p className="mt-4 text-xs font-semibold text-gray-600 text-start">Organization Affiliations</p>
                      {websiteData?.landing?.categories?.organizationAffiliations?.map((item, textIndex) => {
                        return (
                          <div key={textIndex} className="flex gap-4 items-center">
                            <input
                              value={item}
                              ...
                              required
                              onChange={(e) =>
                                handleCategoryArrayInputChange(e.target.value, textIndex, "organizationAffiliations")
                              }
                            />
                            <RiDeleteBinLine
                              onClick={() => deleteCategoryTextIndex(textIndex, "organizationAffiliations")}
                              className={`mt-2 text-xl text-red-500 cursor-pointer`}
                            />
                          </div>
                        );
                    ...
                    <div className="w-full">
                      <p className="mt-4 text-xs font-semibold text-gray-600 text-start">Community Affiliations</p>
                      {websiteData?.landing?.categories?.communityAffiliations?.map((item, textIndex) => {
                        return (
                          <div key={textIndex} className="flex gap-4 items-center">
                            <input
                              value={item}
                              ...
                              required
                              onChange={(e) =>
                                handleCategoryArrayInputChange(e.target.value, textIndex, "communityAffiliations")
                              }
                            />
                            <RiDeleteBinLine
                              onClick={() => deleteCategoryTextIndex(textIndex, "communityAffiliations")}
                              className={`mt-2 text-xl text-red-500 cursor-pointer`}
                            />
                          </div>
                        );
                  ...
                  <>
                    <div className="flex justify-center items-center mt-4 w-full">
                      ...
                      >
                        ...
                        <input id="dropzone-file" type="file" className="hidden" onChange={handleAddImage} />
                      </label>
                    </div>
