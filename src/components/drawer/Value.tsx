...
interface MyValueFieldsProps {
  toggleDrawer: () => void;
}

const MyValueFields: React.FC<MyValueFieldsProps> = ({ toggleDrawer }) => {
  const { websiteData } = useSelector((state: RootState) => state.content);
  const [linkError, setLinkError] = useState<string>("");
  const dispatch = useDispatch();

  const handleInputVisionChange = (fieldName: keyof typeof websiteData.vision, value: string) => {
    const updatedVisionData = { ...websiteData.vision, [fieldName]: value };
    // dispatch(handleWebsiteData({ ...websiteData, vision: updatedVisionData }));
  };

  const handleInputChange = (fieldName: keyof typeof websiteData.value, value: string) => {
    if (fieldName === "buttonLink" && !validateURL(value)) {
      setLinkError("*Invalid Url format");
    } else {
      setLinkError("");
    }
    const updatedMyValueData = { ...websiteData.value, [fieldName]: value };
          ...
          <div className="flex flex-col items-start p-4 mt-4">
            ...
            <div className="w-full">
              <input
                ...
                required
                onChange={(e) => handleInputVisionChange("title", e.target.value)}
              />
            </div>
          </div>
          <div className="flex flex-col items-start p-4 mt-4 mb-20">
            <p className="text-xs font-semibold text-gray-600">Impact Statement</p>
            <div className="w-full">
              <textarea
                ...
                required
                rows={8}
                onChange={(e) => handleInputVisionChange("description", e.target.value)}
              />
            </div>
          </div>
        </form>
