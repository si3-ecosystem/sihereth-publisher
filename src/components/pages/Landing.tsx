...
const Landing = ({ data }: { data: LandingTypes }) => {
  ...
  return (
    <div className="flex flex-col p-10 w-full lg:flex-row text-lg">
      {/* left side  */}
      <div className="w-1/2 pl-20 space-y-6 flex flex-col justify-center items-start">
        ...
        {/* hashtags */}
        <p className="tracking-wider text-2xl leading-4 font-medium text-[#3E21F3]">{data.brandPilars.toUpperCase()}</p>
        <section className="flex flex-wrap gap-4">
          {data.hashTags.map((hashtag) => (
            <div
              key={hashtag}
              className="bg-[#ECE9FD] text-gray-800 tracking-wide w-fit py-3 px-4 rounded-lg text-center hover:text-gray-200 hover:bg-gray-800 transition-all ease-out duration-500 cursor-default hover:shadow-md"
            >
              #{hashtag}
            </div>
          ))}
        ...
        {/* information */}
        <section className="p-5 space-y-4 bg-gray-100 mf:w-fit w-[90%] rounded-lg">
          <div className="flex gap-6 font-medium">
            <p className="text-[#3E21F3] tracking-wider font-medium w-64">Based in:</p>
            <p className="hover:text-[#3E21F3] cursor-default">{data.region}</p>
          </div>
          <div className="flex gap-6 font-medium">
            <p className="text-[#3E21F3] tracking-wider font-medium w-64">Organization Affiliations:</p>
          ...
          <div className="flex gap-6 font-medium">
            <p className="text-[#3E21F3] tracking-wider font-medium w-64">Community Affiliations:</p>
            <p className="hover:text-[#3E21F3] cursor-default flex gap-6">
          ...
          <div className="flex gap-6 font-medium">
            <p className="text-[#3E21F3] tracking-wider font-medium w-64">Superpowers:</p>
            <p className="hover:text-[#3E21F3] cursor-default flex gap-6">
      ...
      <div className="w-1/2 flex justify-center items-center relative">
        <section className="relative">
          ...
          <div className="w-[51%] h-[30%] sm:w-[40%] mf:h-[25%] mf:w-[45%] rounded-[12px] border border-[#C8BAFD] p-4 flex flex-col justify-center items-center gap-2 bg-[#ECE9FD] top-[86%] sm:top-[80%] sm:right-[-5%] mf:top-[85%] mf:right-[10%] absolute right-[-10%] xxl:w-[225px] xxl:h-[114px] xxl:gap-4 xxl:right-[23%] xxl:top-[88%]">
            ...
            <span className="font-dm-sans text-[12px] tracking-wide text-gray-800 sm:text-[14px] xxl:text-base font-normal xxl:leading-6">
